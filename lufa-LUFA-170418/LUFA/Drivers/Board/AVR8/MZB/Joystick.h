...
#define __JOYSTICK_BUMBLEB_H__
	...
	/* Public Interface - May be used in end-application: */
		/* Macros: */
			...
			#define ADC_JOYX		7	//pour ADC0 apres 
			#define ADC_JOYY		6	//pour ADC1 apres
		...
		//	{
				//return ((PINF & BUTTONS_BUTTONSEL) ^ BUTTONS_BUTTONSEL);
			...
			{
				ADCSRA = (1<<ADPS2) | (1<<ADPS1) | (1<<ADPS0); //Maximum resolution 
				ADCSRA |= (1<<ADEN);		//ENABLE ADC
				ADCSRB = (((mux&0x20)==0?0:1)<<MUX5); 	//LINE TO USE
				ADMUX = mux & 0x1f; 			// Choose ADCX
				ADMUX |= (0<<REFS1) | (1<<REFS0);	//Reference to AVCC
				ADMUX |= (1<<ADLAR);		//Left adjusted, sufficient to read ADCH
			}
			...
			*/
			...
			{
				adc_init(ADC_JOYX);
				uint8_t x_axis = adc_read();
				x_axis &= 0xF0;

				adc_init(ADC_JOYY);
				uint8_t y_axis = adc_read();
				y_axis &= 0xF0;
				y_axis = y_axis >>4;
				
				return (uint8_t)(x_axis + y_axis);		
			}
		#endif
